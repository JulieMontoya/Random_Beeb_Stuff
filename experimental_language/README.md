...
The intent is to use tbe BBC Micro's existing BASIC editor to enter a
program, which BBC BASIC can run interpretively; then compile it to produce
a machine code program which, when run, will do the same job.
...
converting decimal and hex numbers to binary, translating variable names
to addresses, parsing expressions and ordering operations by priority,
and so forth.  J-CODE will be as streamlined as possible; for instance,
all numberic constants are already in binary, and all variable accesses
have been replaced by ! constructs.  Everything in BASIC will have a direct
equivalent in J-CODE.
...
with variable accesses translated to ! constructs.

Procedures and functions are always treated first, in order to complete the
symbol table with the addresses of their parameter information blocks.
...
Variables used as parameters within a function or procedure definition have
their existing values saved to the Stack, and new values written to them
from the parameters given on the Stack.  At the end, their values will be
restored.

### LOCAL VARIABLES

Variables declared as `LOCAL` within a function or procedure have their
existing values saved on the Stack, as for parameters, and restored
afterwards; however, they are not yet given new values.

### PROCEDURE / FUNCTION INDICATOR

This byte is 0 for a PROCedure  (which does not return anything)  or 1 for
...
`DIP` DIvide Product performs an **unsigned** division without clearing the
X register beforehand; therefore, if the last operation was *, the 32-bit
product will be used as the dividend.  The divisor must be positive!

`MUF` MUltiply Fractional multiplies two 16-bit values and takes the upper
16 bits of the 32-bit product as the result.  It is equivalent to
multiplying by B/65536.
...
Subscripts start from 0 _and wrap around_, so in the above example `S(320)`
is an alias for `S(0)`; `S(318)`and `S(-2)` refer to the same  (last but
one)  element, and so on.   **This behaviour differs from BBC BASIC.**  You
will need to increase your dimensions by one if you want to use element 0.
...
Array access instructions take the address of this header as their
argument, and expect the required number of subscripts on the Stack.

```
;  example reading Z(4,7)
use 4       ;  first subscript
use 7       ;  second subscript
gea &2824   ;  address of header for Z(,)
;  now top of Stack is equal to Z(4,7)

;  example setting S(X%)=P%
use (&440)  ;  address of P%; () gets value there
use (&460)  ;  address of X%; () gets value there
pua &25A1   ;  address of header for S()
```

## ARRAY BOUNDS ENFORCEMENT

Array bounds are enforced by reducing each subscript modulo the size of its
dimension.  If the remainder is negative, then the size is added  (which is
certain to give a positive answer, by operation of the modulus function).
This seemed nicer to the author than throwing an error on an out-of-bounds
array access, and also took care of the issue with zero-based subscripts
_fairly_ neatly insofar as zero seems to be rarely used anyway.

The J-code interpreter also contains "dangerous" versions of the array
access instructions, which do not include any array bounds checking.  You
may use these at your peril.  
...
(the code pointer is pointing here already), i.e. the top of the loop.
...
The `RND` function from BBC BASIC is not repcreated faithfully, but through
the `rnd` instruction, which takes a single argument and behaves like
`RND(integer)` in BBC BASIC.

The pseudo-random number generator uses a modulus of 65537 and a base
of 75.  The seed  (between 1 and 65536)  is stored in memory with 1
subtracted.  Each new value is calculated from the previous one according
to the following formula:
```
new := ((75 * (old + 1)) % 65537) - 1
```
The new value is multiplied by the function's operand, divided by 65536
and 1 added, to give a value between 1 and the supplied value.

# DIFFERENCES FROM BBC BASIC

+ All mathematics is 16-bit integer.
+ / and DIV behave differently: / takes a positive 32-bit dividend, DIV 16 bits signed.
...
+ RND() uses a 16-bit seed. 

# FOR TOMORROW AND BEYOND

Include some obvious instructions anyway, even if not used by BASIC.  They
may come in handy for implementing extensions, or other languages, or just
manually writing J-CODE.

It would be very, very nice to be able to include support for multi-line
IF statements  (as in YSON with IF / THEN / ELIF / THEN / ELSE / FI),  even
at the expense of compatibility with BBC BASIC.

For portability, will need to add terminal I/O.
